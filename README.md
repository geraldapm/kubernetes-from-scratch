# Kubernetes From Scratch - Provision Kubernetes Cluster from ground zero
...
- crictl
- CRI-O
- Calico CNI with Tigera Operator
...
When using only one single CA certificates pair:
```bash
{
...
for cert in ${kubeca[*]}; do
  ...
  openssl req -new -key "$cert.key" -sha256 \
      ...
      -out "$cert.csr"
  openssl ca -days 3653 -batch -in "$cert.csr" \
      -config "ca.conf" -out "$cert.crt"
  openssl verify -CAfile rootca/ca.crt $cert.crt
  cat $cert.crt \
      rootca/ca.crt > $cert-chain.crt
done

```
Analyze CA cert
```bash
openssl x509 -noout -text -in kubernetes-ca.crt | grep -A4 -i issuer
...
for host in $(cat /etc/hosts | grep "gpmrawk8s-" | awk '{print $2}' ); do
  ...
  scp kubernetes-ca-chain.crt root@${host}:/var/lib/kubelet/ca.crt
  #singleca# scp ca.crt root@${host}:/var/lib/kubelet/
